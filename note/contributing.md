...
Файлы с информацией об именах упомянутых в документах находятся в директории `/name`.
...
(`person`, `place`, `org`) в файле в директории `/name`:
```xml
...
- перечисляет на [странице ошибок](/errors.html#no-ref) имена, закодированные без атрибута `ref`.
...
Шероховатости в кодировке (имена без `ref`, `unclear`, несовпадение имени файла сущности с основным именем) перечисляются на <a href="/errors.html" target="collectionViewer">странице ошибок</a>.
